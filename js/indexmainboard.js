let storemainboard = new Storemainboard("Thành Nhân");
...
function getByName() {
    ...
    let list = storemainboard.getByNameContain(nameSearch);
    let html = ``;
...
function add() {
    ...
    let newProduct = new Productmainboard(img, name, id, price_old, price_new);
    storemainboard.add(newProduct);
    getAll();
...
function getAll() {
    let list = storemainboard.getList();
    let html = ``;
    for (let i = 0; i < list.length; i++) {
        html = html + `
        <div class="sanpham-main">
...
function showFormEdit(index) {
    let oldProduct = storemainboard.getProductByIndex(index);
    ...
    document.getElementById("btn-mainboard").innerHTML = `<button onclick="edit(${index})">Lưu</button> `
}
function edit(index) {
    let img = document.getElementById("img").value;
    let name = document.getElementById("name").value;
    ...
    let price_new = document.getElementById("price_new").value; 
    let newProduct = new Productmainboard(img, name,id, price_old,price_new);
    storemainboard.edit(index, newProduct);
    ...
    document.getElementById("price_old").value = "";
    document.getElementById("price_new").value = "";
     document.getElementById("btn-mainboard").innerHTML = `<button onclick="add()">Thêm mới</button> `
}
getAll()
